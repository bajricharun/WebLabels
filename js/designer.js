...
com.logicpartners.labelDesigner = function (
  canvasid,
  labelWidth,
  labelHeight,
  labelResolution
) {
  this.numOfLabelsToPrint = 1;
  this.canvas = document.getElementById(canvasid);
  this.canvasElement = $(this.canvas);

  this.labelWidth = labelWidth * this.dpi;
  this.labelHeight = labelHeight * this.dpi;
  this.propertyInspector = new com.logicpartners.propertyInspector(
    this,
    this.canvas
  );
  ...
  this.SetNumOfLabels = function (numOfLabels) {
    this.numOfLabelsToPrint = numOfLabels;
  };
  this.ResetNumOfLabels = function () {
    this.numOfLabelsToPrint = 1;
  };

  this.updateLabelSize = function (width, height) {
    var xchange =
      width * this.dpi + 10 - parseInt(this.canvasElement.prop("width"));
    this.labelWidth = width * this.dpi;
    this.labelHeight = height * this.dpi;
    this.canvasElement
      .prop("width", this.labelWidth + 10)
      .prop("height", this.labelHeight + 10);
    this.labelX = this.canvas.width / 2 - this.labelWidth / 2;
    this.labelY = 5;
    //console.log(xchange);
  ...
  this.canvasElement
    ...
    .on("mousedown", function () {
      ...
      if (self.newObject) {
        // Create new object.
        self.elements[self.currentLayer++] = new self.newObject(
          self.dragStartPosition.x,
          self.dragStartPosition.y,
          1,
          1
        );
        self.dragAction = 8;
        self.activeElement = self.elements[self.currentLayer - 1];
        self.newObjectController.button.removeClass(
    ...
    .on("mousemove", function () {
      if (self.dragging && self.activeElement) {
        ...
        switch (self.dragAction) {
          case 0:
            self.move(
              coords.x + self.dragElementOffset.x,
              coords.y + self.dragElementOffset.y
            );
            break;
          default:
            self.resize(
              coords.x - self.dragLastPosition.x,
              coords.y - self.dragLastPosition.y,
              self.dragAction
            );
            break;
        }
  ...
  document.getElementById("upLeft").addEventListener("click", function () {
    if (self.activeElement) {
      self.activeElement.x = 10;
      self.activeElement.y = 10;
      var labelH = self.activeElement.height;
      if (labelH == null) {
        self.activeElement.x = 50;
        self.activeElement.y = 50;
      }
    }
    self.updateCanvas();
    event.preventDefault();
  ...
  document.getElementById("bolder").addEventListener("click", function () {
    if (self.activeElement) {
      self.activeElement.style.fontSize = 500;
    }
  });
  document.getElementById("bottomLeft").addEventListener("click", function () {
    if (self.activeElement) {
      var canvas1 = document.getElementById(canvasid);
      var newvar = canvas1.height;
      var labelH = self.activeElement.height;

      self.activeElement.x = 10;
      self.activeElement.y = newvar - labelH - 11;
      if (labelH == null) {
        self.activeElement.x = 50;
        self.activeElement.y = newvar - 50;
      }
    }
    self.updateCanvas();
    event.preventDefault();
  ...
  document.getElementById("centerLeft").addEventListener("click", function () {
    if (self.activeElement) {
      var canvas1 = document.getElementById(canvasid);
      var newvar = canvas1.height;
      var labelH = self.activeElement.height;
      if (labelH == null) {
        self.activeElement.y = newvar / 2 - 7.5;
        self.activeElement.x = 50;
      } else {
        self.activeElement.x = 10;
        self.activeElement.y = newvar / 2 - labelH / 2;
      }
    }
    self.updateCanvas();
    event.preventDefault();
  ...
  document.getElementById("upRight").addEventListener("click", function () {
    if (self.activeElement) {
      var canvas1 = document.getElementById(canvasid);
      var newvar = canvas1.width;
      var labelH = self.activeElement.height;
      var labelW = self.activeElement.width;
      self.activeElement.y = 10;
      if (labelW == null) {
        self.activeElement.x = newvar - 200;
      } else {
        self.activeElement.x = newvar - labelW - 10;
      }
      if (labelH == null) {
        self.activeElement.x = newvar - 50;
        self.activeElement.y = 50;
      }
    }
    self.updateCanvas();
  ...
  document.getElementById("bottomRight").addEventListener("click", function () {
    if (self.activeElement) {
      var canvas1 = document.getElementById(canvasid);
      var newvar = canvas1.width;
      var canvasH = canvas1.height;
      var labelH = self.activeElement.height;
      var labelW = self.activeElement.width;
      self.activeElement.y = canvasH - labelH - 10;
      ...
      if (labelH == null) {
        self.activeElement.y = canvasH - 50;
        self.activeElement.x = newvar - 50;
      }
    }
    self.updateCanvas();
  ...
  document.getElementById("centerRight").addEventListener("click", function () {
    if (self.activeElement) {
      var canvas1 = document.getElementById(canvasid);
      var newvar = canvas1.height;
      var canvasW = canvas1.width;
      var labelW = self.activeElement.width;
      var labelH = self.activeElement.height;
      self.activeElement.y = newvar / 2 - labelH / 2;

      if (labelW == null) {
        self.activeElement.x = canvasW - 200;
      } else {
        self.activeElement.x = canvasW - labelW - 10;
      }
      if (labelH == null) {
        self.activeElement.x = canvasW - 50;
        self.activeElement.y = newvar / 2 - 7.5;
      }
    }
    self.updateCanvas();
  ...
  document.getElementById("upCenter").addEventListener("click", function () {
    if (self.activeElement) {
      var canvas1 = document.getElementById(canvasid);
      var newvar = canvas1.height;
      var canvasW = canvas1.width;
      ...
      var labelH = self.activeElement.height;
      if (labelW == null) {
        self.activeElement.x = canvasW / 2 - 50;
      } else {
        self.activeElement.x = canvasW / 2 - labelW / 2;
      }
      self.activeElement.y = 10;
      if (labelH == null) {
        self.activeElement.x = canvasW / 2 - 10;
        self.activeElement.y = 50;
      }
    }
    self.updateCanvas();
  ...
  document
    ...
    .addEventListener("click", function () {
      if (self.activeElement) {
        ...
        self.activeElement.y = newvar - labelH - 10;
        if (labelW == null) {
          self.activeElement.x = canvasW / 2 - 50;
        } else {
          self.activeElement.x = canvasW / 2 - labelW / 2;
  ...
  this.addObject = function (object) {
    if (object.GetElementType() == "image") {
      var greatestUniqueID = 1,
        numOfImageElements = 0;

      for (var i = 0; i < this.currentLayer; ++i) {
        if (
          this.elements[i] != null &&
          this.elements[i].GetElementType() == "image"
        ) {
          var currentGreatestUniqueID = parseInt(this.elements[i].uniqueID);
          if (currentGreatestUniqueID > greatestUniqueID)
            greatestUniqueID = currentGreatestUniqueID;

          ++numOfImageElements;
        }
      }

      if (numOfImageElements != 0) {
        object.uniqueID = ++greatestUniqueID;
        object.name = "Image " + greatestUniqueID;
      }
    }
  ...
  this.ClearAllVariableTexts = function () {
    for (var i = 0; i < this.currentLayer; ++i)
      if (
        this.elements[i] &&
        this.elements[i].GetElementType() == "variableText"
      )
        this.elements[i].text = "VarText";
  };
  ...
  this.getHandle = function (coords) {
    ...
    var leftEdge =
      coords.x > this.activeElement.x - 5 &&
      coords.x < this.activeElement.x + 5;
    var rightEdge =
      coords.x > this.activeElement.x + this.activeElement.getWidth() - 5 &&
      coords.x < this.activeElement.x + this.activeElement.getWidth() + 5;
    var topEdge =
      coords.y > this.activeElement.y - 5 &&
      coords.y < this.activeElement.y + 5;
    var bottomEdge =
      coords.y > this.activeElement.y + this.activeElement.getHeight() - 5 &&
      coords.y < this.activeElement.y + this.activeElement.getHeight() + 5;

    var verticalHit =
      coords.y > this.activeElement.y &&
      coords.y < this.activeElement.y + this.activeElement.getHeight();
    var horizontalHit =
      coords.x > this.activeElement.x &&
      coords.x < this.activeElement.x + this.activeElement.getWidth();
  ...
  this.resize = function (xchange, ychange) {
    ...
    if (this.activeElement.width < 0) {
      this.activeElement.x =
        this.activeElement.x + this.activeElement.getWidth();
    ...
    if (this.activeElement.height < 0) {
      this.activeElement.y =
        this.activeElement.y + this.activeElement.getHeight();
  ...
  this.updateCanvas = function () {
    ...
    this.drawingContext.lineWidth = 2;
    if (this.activeElement) this.activeElement.drawActive(this.drawingContext);
  };
  ...
  this.generateZPL = function () {
    ...
    data += "^PQ" + this.numOfLabelsToPrint + "\r\n" + "^XZ\r\n";
  ...
  this.addRectangle = function (x, y, width, height) {
    this.elements[this.currentLayer++] = new this.Rectangle(
      x,
      y,
      width,
      height
