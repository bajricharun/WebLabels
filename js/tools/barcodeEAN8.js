...
com.logicpartners.designerTools.barcodeEAN8 = function()
{
	var self = this;
	this.counter = 1;
	this.button = $("<div></div>").addClass("designerToolbarBarcode designerToolbarButton").attr("title", "Barcode - EAN 8").append($("<div></div>"));
	this.object = function(x, y, width, height, barcodeTitle)
	{
		var width = 100;
		var canvasHolder = $("<canvas></canvas>").prop("width", "100").prop("height", "1");
		this.name = "Barcode2 " + self.counter++;
		this.text = barcodeTitle || "1234567";
		this.x = x;
		this.y = y;
		//this.width = width;
		this.height = height;
		...
		this.toZPL = function(labelx, labely, labelwidth, labelheight) {
			return "^FO" + (this.x - labelx) + "," + (this.y - labely) + "^B8N," + this.height + "N,N^FD" + this.text + "^FS";
		}
		...
		this.GetElementType=function() {return "barcodeEAN8";}
	}
};