...
com.logicpartners.designerTools.variableText = function()
{
	var self = this;
	this.counter = 1;
	this.button = $("<div></div>").addClass("designerToolbarVariableText designerToolbarButton").attr("title", "Variable Text").append($("<div></div>"));
	...
	this.object =  function(x, y, width, height, textTitle)
	{
		...
		this.toZPL = function(labelx, labely, labelwidth, labelheight) {
			return "^FO" + (this.x - labelx) + "," + (this.y - labely) + "^FV" + this.text + "^FS";
		}
		...
		this.GetElementType=function() {return "variableText";}
	}
}
